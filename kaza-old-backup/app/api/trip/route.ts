...
export async function POST(request: NextRequest) {
  try {
    ...
    let destinations: Destination[] = []; // Actually AIResponseDestination[] but cast for convenience

    // 1. Generate Destinations with AI
    if (apiKey) {
      try {
        console.log('Generating destinations with AI...');
      ...
      console.log('No API key found (OPENROUTER_API_KEY), using fallback data');
      destinations = getFallbackDestinations(preferences);
    }
  ...
  return [
    ...
    },
    {
      ...
      estimatedCost: {
        ...
        activities: Math.round(budget * 0.2),
        food: Math.round(budget * 0.2),
        total: Math.round(budget * 0.75) // Very affordable
      },
